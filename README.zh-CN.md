...
该脚本会创建或复用 `.venv`，以本地模式安装 WhyWiki，在能够识别所属 PID 时停止 `127.0.0.1:8765` 上的旧服务，初始化 SQLite 数据库，并在 <http://127.0.0.1:8765> 启动一个新的 Web 应用。如果端口被无法识别的进程占用，脚本会退出并给出手动清理指引，而不是猜测处理。
